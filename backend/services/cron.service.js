...
class CronService {
  ...
  static async generateDailyTimesheets(targetDate = null) {
    ...
    try {
      // 1. Lấy tất cả nhân viên active có department
      ...
      if (employees.length === 0) {
        console.log('ℹ️ [CRON] No active employees found');
        return { created: 0, skipped: 0, errors: 0 };
      }

      console.log(`📊 [CRON] Processing ${employees.length} employees for ${workDate}`);

      let created = 0;
      let skipped = 0;
      let errors = 0;

      // 2. Lấy work shifts cho tất cả departments
      ...
      for (const emp of employees) {
        try {
          ...
          if (!shiftId) {
            console.warn(`⚠️ [CRON] No work shift for department ${emp.department_id}`);
            skipped++;
            continue;
          }
          ...
          console.error(`❌ Error creating timesheet for ${emp.employee_id}:`, err.message);
          errors++;
        }
      }

      console.log(`✅ [CRON] Created: ${created}, Skipped: ${skipped}, Errors: ${errors}`);
  ...
  static async manualTrigger(date) {
    ...
    console.log(`🔧 [MANUAL] Generating timesheets for ${targetDate}`);
